...
static void BIP32Fingerprint(const uint8_t *public_key, uint8_t *public_key_id) {
    unsigned char tmp_hash[MYC_SHA3_256_DIGEST_LENGTH];
    const uint8_t prefix = 0x03;
    MYC_SHA3_CTX sha3_ctx;

    // First 4 bytes of RIPEMD160(SHA3-256(0x03 + public key))
    myc_sha3_256_Init(&sha3_ctx);
    myc_sha3_Update(&sha3_ctx, &prefix, 1);
    myc_sha3_Update(&sha3_ctx, public_key, 32);
    myc_sha3_Final(&sha3_ctx, tmp_hash);
...
{
    unsigned char num[4];
    myc_hmac_sha3_512_ctx hmac_ctx;
    ...
    myc_hmac_sha3_512_init(&hmac_ctx, chainCode, 32);
    myc_hmac_sha3_512_update(&hmac_ctx, &header, 1);
    myc_hmac_sha3_512_update(&hmac_ctx, data, 32);
    myc_hmac_sha3_512_update(&hmac_ctx, num, sizeof(num));
    myc_hmac_sha3_512_final(&hmac_ctx, output, 64);
}
...
int ed25519_init_seed(const uint8_t *seed, int seed_len, uint8_t *private_key, uint8_t *chaincode) {
    ...
    myc_hmac_sha3_512_ctx hmac_ctx_ed25519;

    // MAC result
    unsigned char tmp[64] = {0};

    myc_hmac_sha3_512_init(&hmac_ctx_ed25519, hashkey_ed25519, sizeof(hashkey_ed25519));
    myc_hmac_sha3_512_update(&hmac_ctx_ed25519, seed, seed_len);
    myc_hmac_sha3_512_final(&hmac_ctx_ed25519, tmp, sizeof(tmp));

    // checking the private key condition
    if(!myc_ed25519_verify_master_hash(tmp)) {
        return 0;
    }
...
int ed25519_derive_priv(const uint8_t *chainCode, const uint8_t *public_key, const uint8_t *private_key, uint8_t *child_fingerprint, uint8_t *childCode, uint8_t *child_private_key, unsigned int nChild) {
    ...
    myc_ed25519_add_scalar(NULL, child_private_key, tmp);
...
int ed25519_derive_pub(const uint8_t *chainCode, const uint8_t *public_key, uint8_t *child_fingerprint, uint8_t *childCode, uint8_t *child_public_key, unsigned int nChild) {
    ...
    myc_ed25519_add_scalar(child_public_key, NULL, tmp);
